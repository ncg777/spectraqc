...
def _render_qcreport_viewer_section(
    ...
    viewer_js = (
        ...
        "  });"
        "  const xAxisLabel=state.type==='line'?(state.useLog?'Frequency (log10 Hz)':'Frequency (Hz)'):'Index';"
        ...
        "    }else{"
        "      noteEl.textContent=`x ${formatNumber(viewport.xMin+1)}-${formatNumber(viewport.xMax+1)}, y ${formatNumber(viewport.yMin)}-${formatNumber(viewport.yMax)}${state.units?` ${state.units}`:''}`;"
        "    }"
        "  }"
