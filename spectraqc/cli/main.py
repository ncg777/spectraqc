...
VIEWPORT_JS_HELPER = (
    ...
    "    const xMin=Math.min(v.xMin,v.xMax);"
    "    const xMax=Math.max(v.xMin,v.xMax);"
    "    const yMin=Math.min(v.yMin,v.yMax);"
    "    const yMax=Math.max(v.yMin,v.yMax);"
    "    return {xMin,xMax,yMin,yMax};"
    "  }"
    "  function clampViewport(viewport,bounds,minSpan){"
    "    const v=normalizeViewport(viewport);"
    "    const rangeX=bounds.xMax-bounds.xMin;"
    "    const rangeY=bounds.yMax-bounds.yMin;"
    "    const minSpanX=Math.min(Math.max(minSpan.x||0,0),rangeX);"
    "    const minSpanY=Math.min(Math.max(minSpan.y||0,0),rangeY);"
    "    const spanX=Math.min(Math.max(v.xMax-v.xMin,minSpanX),rangeX);"
    "    const spanY=Math.min(Math.max(v.yMax-v.yMin,minSpanY),rangeY);"
    "    const centerX=(v.xMin+v.xMax)/2;"
    "    const centerY=(v.yMin+v.yMax)/2;"
    "    let xMin=centerX-spanX/2;"
    "    let xMax=centerX+spanX/2;"
    "    let yMin=centerY-spanY/2;"
    "    let yMax=centerY+spanY/2;"
    "    if(xMin<bounds.xMin){xMax+=bounds.xMin-xMin; xMin=bounds.xMin;}"
    "    if(xMax>bounds.xMax){xMin-=xMax-bounds.xMax; xMax=bounds.xMax;}"
    "    if(yMin<bounds.yMin){yMax+=bounds.yMin-yMin; yMin=bounds.yMin;}"
    "    if(yMax>bounds.yMax){yMin-=yMax-bounds.yMax; yMax=bounds.yMax;}"
    "    return {xMin,xMax,yMin,yMax};"
    "  }"
    "  function zoomViewport(viewport,scale,center,bounds,minSpan){"
    "    const v=normalizeViewport(viewport);"
    "    const spanX=(v.xMax-v.xMin)*scale.x;"
    "    const spanY=(v.yMax-v.yMin)*scale.y;"
    "    const next={"
    "      xMin:center.x-spanX/2,"
    "      xMax:center.x+spanX/2,"
    "      yMin:center.y-spanY/2,"
    "      yMax:center.y+spanY/2"
    "    };"
    "    return clampViewport(next,bounds,minSpan);"
    "  }"
    ...
    "      yMax:v.yMax+delta.y"
    "    };"
    "    return clampViewport(next,bounds,{x:0,y:0});"
    "  }"
    ...
    "    };"
    "    return clampViewport(rect,bounds,minSpan);"
    "  }"
    "  return {normalizeViewport,clampViewport,zoomViewport,panViewport,viewportFromRect};"
    "})();"
    "if(typeof module!=='undefined'){module.exports={ViewportMath};}"
)
...
def _render_qcreport_viewer_section(
    ...
    viewer_css = (
        ...
        ".viewer-chart canvas{width:100%;height:auto;touch-action:none}"
        ".viewer-chart-toolbar{display:flex;align-items:center;justify-content:space-between;gap:8px;flex-wrap:wrap}"
        ".viewer-chart-controls{display:flex;gap:6px;flex-wrap:wrap}"
        ".viewer-chart-controls button{border:1px solid #ddd;background:#fafafa;border-radius:4px;padding:3px 8px;cursor:pointer;font-size:12px}"
        ".viewer-chart-controls button:hover{background:#f0f0f0}"
        ...
        ".viewer-overlay-body canvas{max-width:100%;height:auto}"
    )
    viewer_js = (
        "<script>"
        f"{VIEWPORT_JS_HELPER}"
        ...
        "  if(overlayChartState && overlayChartState.overlayInstance){"
        "    overlayChartState.instances.delete(overlayChartState.overlayInstance);"
        "    overlayChartState.overlayInstance=null;"
        "  }"
        "  overlayChartState=null;"
        "}"
        ...
        "}"
        "function getChartMetrics(state,canvas){"
        "  const padding={top:24,right:16,bottom:56,left:64};"
        "  const w=canvas.width-padding.left-padding.right;"
        "  const hgt=canvas.height-padding.top-padding.bottom;"
        "  const viewport=state.viewport;"
        "  const xMin=state.useLog?Math.log10(viewport.xMin):viewport.xMin;"
        "  const xMax=state.useLog?Math.log10(viewport.xMax):viewport.xMax;"
        "  const xRange=(xMax-xMin)||1;"
        "  const yRange=(viewport.yMax-viewport.yMin)||1;"
        "  return {padding,w,hgt,viewport,xMin,xMax,xRange,yRange};"
        "}"
        "function canvasToData(state,canvas,pt){"
        "  const {padding,w,hgt,viewport,xMin,xRange,yRange}=getChartMetrics(state,canvas);"
        "  const relX=(pt.x-padding.left)/w;"
        "  const relY=(pt.y-padding.top)/hgt;"
        "  const rawX=xMin+(relX*xRange);"
        "  const x=state.useLog?Math.pow(10,rawX):rawX;"
        "  const y=viewport.yMax-(relY*yRange);"
        "  return {x,y};"
        "}"
        ...
        "  };"
        "}"
        "function renderChartInstance(state,instance){"
        "  const {canvas,noteEl}=instance;"
        ...
        "      if(p.x<viewport.xMin||p.x>viewport.xMax) return;"
        "      const rawX=state.useLog?Math.log10(p.x):p.x;"
        "      const x=padding.left+((rawX-xMin)/xRange)*w;"
        "      const y=padding.top+((viewport.yMax-p.y)/yRange)*hgt;"
        ...
        "    const startIndex=Math.max(0,Math.floor(viewport.xMin));"
        "    const endIndex=Math.min(values.length-1,Math.ceil(viewport.xMax));"
        "    const visibleCount=Math.max(1,(endIndex-startIndex+1));"
        "    const barWidth=w/visibleCount;"
        "    const yZero=padding.top+((viewport.yMax-0)/yRange)*hgt;"
        "    for(let i=startIndex;i<=endIndex;i++){"
        "      const v=values[i];"
        "      const x=padding.left+(i-startIndex)*barWidth+4;"
        "      const barH=Math.abs(v)*hgt/yRange;"
        ...
        "    const t=viewport.yMin+(i*(yRange/yTicks));"
        "    const y=padding.top+((viewport.yMax-t)/yRange)*hgt;"
        ...
        "    const label=formatNumber(t)+(state.units?` ${state.units}`:'');"
        "    ctx.fillText(label,4,y+4);"
        "  }"
        "  const xTickVals=[viewport.xMin, (viewport.xMin+viewport.xMax)/2, viewport.xMax];"
        "  ctx.fillStyle='#666'; ctx.font='12px Arial,Helvetica,sans-serif';"
        "  xTickVals.forEach((xVal)=>{"
        "    const raw=state.useLog?Math.log10(xVal):xVal;"
        ...
        "    const label=state.type==='line'?`${formatNumber(xVal)} Hz`:formatNumber(xVal+1);"
        "    ctx.fillText(label,x-14,padding.top+hgt+18);"
        "  });"
        "  if(state.type==='bar'){"
        "    const startIndex=Math.max(0,Math.floor(viewport.xMin));"
        "    const endIndex=Math.min(state.labels.length-1,Math.ceil(viewport.xMax));"
        "    const visibleCount=Math.max(1,(endIndex-startIndex+1));"
        "    const barWidth=w/visibleCount;"
        "    const labelStep=Math.max(1,Math.ceil(visibleCount/10));"
        "    for(let i=startIndex;i<=endIndex;i++){"
        "      if((i-startIndex)%labelStep!==0) continue;"
        "      const x=padding.left+(i-startIndex)*barWidth+4;"
        "      ctx.save();"
        "      ctx.translate(x, padding.top+hgt+28);"
        "      ctx.rotate(-0.35);"
        "      ctx.fillStyle='#555';"
        "      ctx.fillText(formatLabel(state.labels[i]||String(i+1)),0,0);"
        "      ctx.restore();"
        "    }"
        "  }"
        "  const xAxisLabel=state.type==='line'?(state.useLog?'Frequency (log10 Hz)':'Frequency (Hz)'):'Band';"
        "  const yAxisLabel=state.units?`Value (${state.units})`:'Value';"
        ...
        "  if(noteEl){"
        "    if(state.type==='line'){"
        "      const xLabel=state.useLog?'log10 Hz':'Hz';"
        "      noteEl.textContent=`${xLabel} ${formatNumber(viewport.xMin)}-${formatNumber(viewport.xMax)}, y ${formatNumber(viewport.yMin)}-${formatNumber(viewport.yMax)}${state.units?` ${state.units}`:''}`;"
        "    }else{"
        "      const startIndex=Math.max(0,Math.floor(viewport.xMin));"
        "      const endIndex=Math.min(state.labels.length-1,Math.ceil(viewport.xMax));"
        "      const startLabel=state.labels[startIndex]||String(startIndex+1);"
        "      const endLabel=state.labels[endIndex]||String(endIndex+1);"
        "      noteEl.textContent=`bands ${startLabel} - ${endLabel}, y ${formatNumber(viewport.yMin)}-${formatNumber(viewport.yMax)}${state.units?` ${state.units}`:''}`;"
        "    }"
        "  }"
        "  if(state.drag && state.drag.canvas===canvas && state.drag.mode==='zoom' && state.drag.current){"
        "    const start=state.drag.start;"
        "    const curr=state.drag.current;"
        "    const x=Math.min(start.x,curr.x);"
        "    const y=Math.min(start.y,curr.y);"
        "    const rectW=Math.abs(start.x-curr.x);"
        "    const rectH=Math.abs(start.y-curr.y);"
        "    ctx.save();"
        ...
        "  state.instances.forEach((instance)=>renderChartInstance(state,instance));"
        "}"
        "function updateViewport(state,next){"
        "  state.viewport=ViewportMath.clampViewport(next,state.bounds,state.minSpan);"
        "  renderAll(state);"
        "}"
        "function addChartInstance(state,canvas,noteEl){"
        "  const instance={canvas,noteEl};"
        "  state.instances.add(instance);"
        "  return instance;"
        "}"
        "function attachCanvasInteractions(state,canvas){"
        "  canvas.addEventListener('contextmenu',(e)=>e.preventDefault());"
        "  canvas.addEventListener('pointerdown',(e)=>{"
        "    const start=getCanvasPoint(canvas,e);"
        ...
        "    const current=getCanvasPoint(canvas,e);"
        "    if(state.drag.mode==='pan'){"
        "      const {w,hgt,xRange,yRange}=getChartMetrics(state,canvas);"
        "      const deltaX=current.x-state.drag.last.x;"
        "      const deltaY=current.y-state.drag.last.y;"
        "      const dx=-deltaX/w*xRange;"
        "      const dy=deltaY/hgt*yRange;"
        "      updateViewport(state,ViewportMath.panViewport(state.viewport,{x:dx,y:dy},state.bounds));"
        "      state.drag.last=current;"
        "    }else{"
        ...
        "      const dx=Math.abs(current.x-state.drag.start.x);"
        "      const dy=Math.abs(current.y-state.drag.start.y);"
        "      if(dx>6 && dy>6){"
        "        const startData=canvasToData(state,canvas,state.drag.start);"
        "        const endData=canvasToData(state,canvas,current);"
        "        updateViewport(state,ViewportMath.viewportFromRect(startData,endData,state.bounds,state.minSpan));"
        "      }"
        "    }"
        "    state.drag=null;"
        "    renderAll(state);"
        ...
        "    const scale=delta>0?1.15:0.87;"
        "    const cursor=canvasToData(state,canvas,getCanvasPoint(canvas,e));"
        "    updateViewport(state,ViewportMath.zoomViewport(state.viewport,{x:scale,y:scale},cursor,state.bounds,state.minSpan));"
        "  },{passive:false});"
        "}"
        "function buildChartWrapper(state,options){"
        ...
        "  toolbar.className='viewer-chart-toolbar';"
        "  const controls=document.createElement('div');"
        "  controls.className='viewer-chart-controls';"
        "  const zoomIn=document.createElement('button');"
        "  zoomIn.type='button';"
        "  zoomIn.textContent='Zoom in';"
        "  zoomIn.addEventListener('click',()=>{"
        "    const center={x:(state.viewport.xMin+state.viewport.xMax)/2,y:(state.viewport.yMin+state.viewport.yMax)/2};"
        "    updateViewport(state,ViewportMath.zoomViewport(state.viewport,{x:0.8,y:0.8},center,state.bounds,state.minSpan));"
        "  });"
        ...
        "    const center={x:(state.viewport.xMin+state.viewport.xMax)/2,y:(state.viewport.yMin+state.viewport.yMax)/2};"
        "    updateViewport(state,ViewportMath.zoomViewport(state.viewport,{x:1.25,y:1.25},center,state.bounds,state.minSpan));"
        "  });"
        "  const reset=document.createElement('button');"
        "  reset.type='button';"
        "  reset.textContent='Reset';"
        "  reset.addEventListener('click',()=>updateViewport(state,state.bounds));"
        ...
        "    expand.addEventListener('click',()=>openOverlayForChart(state));"
        ...
        "  const instance=addChartInstance(state,canvas,note);"
        "  if(options.isOverlay){"
        "    state.overlayInstance=instance;"
        "  }"
        "  attachCanvasInteractions(state,canvas);"
        "  renderChartInstance(state,instance);"
        "  return wrapper;"
        "}"
        "function openOverlayForChart(state){"
        "  overlayBody.innerHTML='';"
        "  overlayTitle.textContent=state.title||'Chart';"
        "  const wrapper=buildChartWrapper(state,{canvasWidth:900,canvasHeight:500,showExpand:false,isOverlay:true});"
        "  overlayBody.appendChild(wrapper);"
        "  overlay.classList.add('active');"
        "  overlay.setAttribute('aria-hidden','false');"
        "  overlayChartState=state;"
        "}"
        "function createLineChart(container,title,xs,ys,units,useLog){"
        "  const points=[];"
        ...
        "  const rangeX=(maxX-minX)||Math.abs(maxX||1);"
        "  const rangeY=(maxY-minY)||Math.abs(maxY||1);"
        "  const bounds={xMin:minX,xMax:maxX,yMin:minY,yMax:maxY};"
        "  const minSpan={x:rangeX*0.05||1,y:rangeY*0.05||1};"
        "  const state={"
        ...
        "    drag:null"
        "  };"
        "  chartRegistry.set(state.id,state);"
        "  const wrapper=buildChartWrapper(state,{canvasWidth:560,canvasHeight:260,showExpand:true,isOverlay:false});"
        "  container.appendChild(wrapper);"
        "}"
        "function createBarChart(container,title,labels,values,units){"
        "  if(!labels.length||!values.length) return;"
        "  const minVal=Math.min(0,...values);"
        "  const maxVal=Math.max(0,...values);"
        "  const xMax=Math.max(0,values.length-1);"
        "  const rangeX=(xMax-0)||1;"
        "  const rangeY=(maxVal-minVal)||Math.abs(maxVal||1);"
        "  const bounds={xMin:0,xMax:xMax,yMin:minVal,yMax:maxVal};"
        "  const minSpan={x:Math.max(1,rangeX*0.2),y:rangeY*0.05||1};"
        "  const state={"
        "    id:`chart-${++chartCounter}`,"
        "    type:'bar',"
        "    title,"
        ...
        "    createLineChart(charts,'LTPSD Mean',grid,ltpsd,'dB',true);"
        "  }"
        "  if(grid.length && deviation.length){"
        "    createLineChart(charts,'Deviation Curve',grid,deviation,'dB',true);"
        "  }"
        ...
        "  }"
        "  if(meanValues.length){createBarChart(charts,'Band Mean Deviation',meanLabels,meanValues,'dB');}"
        "  if(maxValues.length){createBarChart(charts,'Band Max Deviation',maxLabels,maxValues,'dB');}"
        "  if(varValues.length){createBarChart(charts,'Band Variance Ratio',varLabels,varValues,'');}"
        ...
        "    createBarChart(charts,'Global Metrics',gLabels,gValues,units);"
        "  }"
        "}"
        "loadAllBatchReports();"
        "</script>"
